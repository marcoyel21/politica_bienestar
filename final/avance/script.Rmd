...
Regresion  RD simple: same slope, with a jump (en cero), for all Y (educational % change)

Vamos a hacer una variable dummy para indicar si es mayor a nuestro punto de corte o no

```{r, results='asis'}

data$cutpoint<-as.numeric(data$cambio_internet>0)

data1<-filter(data,pobtot<2500)
attach(data1)
...
## error/ no sale la recta de tratamiento

```{r, results='asis'}

simple.rd<- lm(cambio_esp_top~cambio_internet+cutpoint,data=data)

data$pred<- predict(simple.rd)

plot(cambio_internet,cambio_esp_top,pch=17, frame.plot = FALSE,
     xaxs="i",yaxs="i",
     xlim=c(-20,70), ylim=c(-20,70),
     main = "Rd simple ",
     xlab = "Cambio tasa internet", ylab =" Cambio tasa Calis ", cex.main=1, font.main=1)
with(subset(data1, cutpoint<=0),lines(cambio_internet, pred, col="red", lty=3, lwd=3))
with(subset(data1, cutpoint>1),lines(cambio_internet, pred, col="chocolate1", lty=3, lwd=3)))
```

## Modelo polinomial

y si hacemos una variable que sea cambio_internet & cambio_pc > 0 ?
...
notamos que entre regresion polinomial y spacings estimators no cambia mucho, entonces nos
quedamos con regresion polinomial
pero si cambia entre evenly spaced y quantile spaced method. Se ven mas concentrados alrededor del 
cero en la quantile spaced. eso es lo que querriamos, para que nuestro LATE tenga validez para mas
observaciones? porque el ajuste (la linea roja) no cambia
En Cattaneo dice

```{r, results='asis'}

out<-rdplot(cambio_esp_top,cambio_internet,nbins=c(20,20), binselect = "es")

summary(out)

out3<- rdplot(cambio_mat_top,cambio_internet,nbins=c(20,20), binselect = "es")
summary(out3)

out5<- rdplot(cambio_esp_top,cambio_internet,nbins=c(20,20), binselect = "qs")
summary(out5)

out6<- rdplot(cambio_mat_top,cambio_internet,nbins=c(20,20), binselect = "qspr")
summary(out6)
```


## Estimador del LATE (local ATE) 
## Estimador Kernel uniforme

Obtendremos el estimador puntual del RD con polinomios locales. Utilizamos MCO porque este tipo de estimacion es un ajuste de minimos cuadrados ponderados.
Escogemos un ancho de banda de w=10 por ahora. Todas las observaciones afuera de [w-c,w+c] son excluidas, y las que caen dentro del intervalo pesan lo mismo.
```{r, results='asis'}

out = lm(cambio_esp_top[cambio_internet < 0 & cambio_internet >=-10] ~ cambio_internet[cambio_internet < 0 & cambio_internet >= -10])
...
```
** ojo hay que cambiar la Y

Interpretacion con lo q tenemos ahorita: La interpretacion es que dentro del intervalo de 10 puntos porcentuales, el porcentaje de calificaciones de espanol que *cambia*  disminuye en -.38 puntos porcentuales debido al acceso a Internet: X% de las escuelas de las localidades rurales tuvieron un cambio en el acceso a internet de -10 pp, mientras que X% de las escuelas de localidades rurales tuvieron un cambio en el acceso a Internet de hasta 10 pp
...
## Estimador LATE - kernel triangular

#primero creamos los pesos

w<- NA
w[cambio_internet<0 & cambio_internet>=-10]= 1-abs(cambio_internet[cambio_internet<0 & cambio_internet>=-10]/10)
w[cambio_internet>=0 & cambio_internet<=10]= 1-abs(cambio_internet[cambio_internet>=0 & cambio_internet<=10]/10)

out = lm(cambio_esp_top[cambio_internet < 0] ~ cambio_internet[cambio_internet < 0],weights=w[cambio_internet<0])
left_intercept = out$coefficients[1]
print(left_intercept)
...
#incorporating those terms in the estimation will reduce the approximation error and thus lead
#to changes in the estimated eect.
#pero nosotros lo q vemos abajo es que a medida que aumentas el polinomia mas crecen los IC, y mas chico se hace el valor p

# Ahora con errores estandar apropiados. uniforme. corte 20, polinomio de orden 1, band width de 5,
out = rdrobust(cambio_esp_top, cambio_internet, kernel = "uniform", p = 1, c=20,h = 5)
summary(out)
...
1890/956 = 1.97
1428/559 = 2.55
#estimador varianza covarianza es de NN (?)

# efecto de tratamiento es de .835 (coeficiente)

#ahora triangular.
out = rdrobust(cambio_esp_top, cambio_internet, kernel = "triangular", p = 1, c=20,h = 5)
summary(out)

# tao es de 1.038

# seleccionar ancho de banda optimo

out = rdbwselect(cambio_esp_top, cambio_internet, kernel = "triangular", p = 1, bwselect = "mserd")
summary(out)

#main bandwidth - 4.273

out = rdbwselect(cambio_esp_top, cambio_internet,c=20, kernel = "triangular", p = 1, bwselect = "mserd")
summary(out)
...
out = rdrobust(cambio_esp_top, cambio_internet,c=20, kernel = "triangular", p = 1, bwselect = "mserd")
