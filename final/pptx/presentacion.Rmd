...
Media.C<-c("1.578","43.002","1.906")
Media.T<-c("1.877", "43.302","1.852")
Diferencia<-c("0.299", ".299","0.0000")
Valor.p<-c("0.0000001", "0.2607","0.0004")
dataf<-data.frame(Variable, Media.C, Media.T,Diferencia,Valor.p, stringsAsFactors=TRUE)
kable(dataf, booktabs=T,"latex") %>% kable_styling(position = "center")
```

Dado que las covariables no estan balanceadas dentro del punto de corte y el ancho del intervalo [c-h,c+h], no las incluiremos en la estimacion, ya que... Cattaneo
...
```{r, echo=FALSE,message=FALSE,warning=FALSE,header=FALSE}
out = rdrobust(data$cambio_esp_top,data$cambio_internet, kernel = "triangular", c=15,p = 4, bwselect = "mserd",cluster=data$ClaveMun)
summary(out)
```

```{r, echo=FALSE,message=FALSE,warning=FALSE,header=FALSE}
out = rdrobust(data$cambio_mat_top,data$cambio_internet, kernel = "triangular", c=15,p = 4, bwselect = "mserd",cluster=data$ClaveMun)
summary(out)
```
