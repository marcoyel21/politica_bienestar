...
#Preparacion
censo_10$ENTIDAD<-as.numeric(censo_10$ENTIDAD)
censo_10$MUN<-as.numeric(censo_10$MUN)
censo_10$LOC<-as.numeric(censo_10$LOC)

#Creo indice INEGI

censo<-censo%>%mutate(MUN=sprintf("%03d",censo$MUN),
                      ENT=sprintf("%02d",censo$ENTIDAD),
...
#Creo indice INEGI
censo_10<-censo_10%>%mutate(MUN=sprintf("%03d",censo_10$MUN),
                      ENT=sprintf("%02d",censo_10$ENTIDAD),
                      LOC=sprintf("%04d",censo_10$LOC)) %>% 
              mutate(a=paste(ENT,MUN,sep=""))%>% 
              mutate(INEGI=paste(a,LOC,sep=""))
...
names2<-c('vivtot','tvivpar','vivparhab', #localidad
         'vph_inter','vph_pc') #internet y laptop
for (i in names2){
...
censo_localidad10<-censo_10 %>% 
  select(Entidad,Municipio,Localidad,     #localidad           
         vivtot,tvivpar,vivpar_hab,       # vivienda
         vph_inter,vph_pc)   %>%    # internet y laptop
  mutate(internet=vph_inter*100/vivtot,
         lap=vph_pc*100/vivtot)
...
# Ahora para planea 2010

planea_localidad10<-planea_10 %>% 
  ...
  summarize(esp_1 = mean(POR_N1_ESP_3), 
            esp_II= mean(POR_N2_ESP_3),
            esp_1II = mean(POR_N3_ESP_3), 
            esp_IV= mean(POR_N4_ESP_3),
            mat_1 = mean(POR_N1_MAT_3), 
            mat_II= mean(POR_N2_MAT_3),
            mat_1II = mean(POR_N3_MAT_3), 
            mat_IV= mean(POR_N4_MAT_3),) %>%
  mutate(top_50_esp10=esp_1II+esp_IV,
         top_50_mat10=mat_1II+mat_IV) #convierto a mi variable de interes (porcentaje de niños en la mejor mitad)

#cambio todo a mayus
planea_localidad10 <- as.data.frame(t(apply(planea_localidad10,1,toupper)))
