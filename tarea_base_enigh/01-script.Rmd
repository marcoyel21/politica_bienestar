...
#______________________________________________________________________________
#DATA wranwilng para STATA (version A)
data<-original%>% 
  ...
  mutate(p_leche=18.72,s_leche=leche/alimentos, q_leche=leche/18.72,
         ...
         q_other=30,s_other=(alimentos-leche-huevo)/alimentos, p_other=30/s_other)
...
#DATA wranwilng para STATA (version B)
data_beta<-original%>% 
  select(folioviv,foliohog,est_socio,leche,huevo,pescado)%>%
  mutate(alimentos=(leche+huevo+pescado),
         p_leche=18.72,s_leche=leche/alimentos, q_leche=leche/18.72,
         p_huevo=28.5,q_huevo=huevo/28.5,s_huevo=huevo/alimentos,
         p_pescado=40,q_pescado=pescado/40,s_pescado=pescado/alimentos)


#para correción de precios y shares 
data_beta[mapply(is.infinite, data_beta)] <- NA
data_beta[is.na(data_beta)] <- 0

#Quito hogares que no gastaron nada en alimentos
data_beta<-data_beta %>% filter(alimentos !=0)

# Escribo la base
write_csv(data_beta,"stata_beta.csv")


#______________________________________________________________________________
#______________________________________________________________________________

#DATA wranwilng para STATA (version c)
data_beta<-original%>% 
  select(folioviv,foliohog,est_socio,leche,huevo,pescado)%>%
  mutate( p_leche=18.72, p_huevo=28.5)

# Escribo la base
write_csv(data_beta,"stata_c.csv")

```

```{r}
data_beta<-data_beta %>% mutate(is=paste(folioviv,foliohog))

sum(as.numeric(duplicated(data_beta[,-1])))
...
\begin{table}[htbp]\centering
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
\caption{Elasticidades precio compensadas para población de ingreso bajo}
\begin{tabular}{l*{1}{c}}
\hline\hline
                    &\multicolumn{1}{c}{(1)}\\
                    &\multicolumn{1}{c}{} \\
...
\begin{table}[htbp]\centering
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
\caption{Elasticidades precio compensadas para población de ingreso alto}
\begin{tabular}{l*{1}{c}}
\hline\hline
                    &\multicolumn{1}{c}{(1)}\\
                    &\multicolumn{1}{c}{} \\
...
\begin{table}[htbp]\centering
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
\caption{Elasticidades precio no compensadas para población de ingreso bajo}
\begin{tabular}{l*{1}{c}}
\hline\hline
                    &\multicolumn{1}{c}{(1)}\\
                    &\multicolumn{1}{c}{} \\
...
\begin{table}[htbp]\centering
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
\caption{Elasticidades precio no compensadas para población de ingreso alto}
\begin{tabular}{l*{1}{c}}
\hline\hline
                    &\multicolumn{1}{c}{(1)}\\
                    &\multicolumn{1}{c}{} \\
...
\begin{table}[htbp]\centering
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
\caption{Elasticidades ingreso para población de ingreso bajo}
\begin{tabular}{l*{1}{c}}
\hline\hline
                    &\multicolumn{1}{c}{(1)}\\
                    &\multicolumn{1}{c}{} \\
...
##Elasticidades ingreso para población de ingreso alto

\begin{table}[htbp]\centering
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
\caption{Elasticidades ingreso para población de ingreso alto}
\begin{tabular}{l*{1}{c}}
\hline\hline
                    &\multicolumn{1}{c}{(1)}\\
                    &\multicolumn{1}{c}{} \\
